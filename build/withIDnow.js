...
async function editPodfile(config, action) {
	const podfilePath = (0, path_1.join)(
...
function addRNIdNowFiles({
	...
	const sourceRoot =
		config_plugins_1.IOSConfig.Paths.getSourceRoot(projectRoot);
...
const withIDnow = (expoConfig) => {
	expoConfig = (0, config_plugins_1.withMainApplication)(
		expoConfig,
		(config) => {
	...
	expoConfig = (0, config_plugins_1.withAndroidManifest)(
		expoConfig,
		async (config) => {
	...
	expoConfig = (0, config_plugins_1.withSettingsGradle)(
		expoConfig,
		(config) => {
	...
	expoConfig = (0, config_plugins_1.withAppBuildGradle)(
		expoConfig,
		(config) => {
	...
	expoConfig = (0, config_plugins_1.withProjectBuildGradle)(
		expoConfig,
		(config) => {
			config.modResults.contents = applyRepositories(
				config.modResults.contents
			);
			return config;
		}
